    ...
    try {
      ...
      entries.forEach(e => {
        ...
        if (!projectTimeMap.has(e.project_id)) {
          projectTimeMap.set(e.project_id, {
            name: e.project.name,
      ...
      let days: Date[] = [];
      let formatStr = 'EEE';
      if (dateFilter === 'day') { days = [today]; formatStr = 'dd MMM'; }
      else if (dateFilter === 'month') { days = eachDayOfInterval({ start: new Date(periodStart), end: new Date(periodEnd) }); formatStr = 'dd'; }
      else { days = eachDayOfInterval({ start: new Date(periodStart), end: new Date(periodEnd) }); formatStr = 'EEE'; }
      ...
      entries.slice(0, 8).forEach(e => {
        timeline.push({
          id: `te-${e.id}`, type: 'time',
          date: new Date(e.start_time),
          text: `Temps tracké sur ${e.project?.name || 'Projet'}`,
          details: fmt_hms(e.duration_minutes || 0),
        });
      });
      ...
      (recentNotes || []).forEach(n => {
        timeline.push({
          ...
          text: 'Note créée 📝',
          details: n.content?.slice(0, 60) + (n.content?.length > 60 ? '…' : ''),
        });
      });
      ...
      (recentPlannings || []).forEach(p => {
        timeline.push({
          ...
          text: 'Planning créé 📅',
          details: p.name || 'Nouveau planning',
        });
      });

      createdProjects
        .filter(p => p.created_at >= periodStart && p.created_at <= periodEnd)
        .forEach(p => {
      ...
      const { count: planningSheetsCount } = await supabase
        .from('planning_sheets')
  ...
  return (
    <div className="min-h-screen bg-[#f4f5f7] dark:bg-slate-950">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">
                  {dateFilter === 'day' ? "Aujourd'hui" : dateFilter === 'week' ? 'Cette semaine' : 'Ce mois'}
                </p>
              </div>
              {data?.activeSessionStartTime && (
        ...
        {/* ── Row 2b: Projets donut compact + Points & Rang ── */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
              {data?.currentTasks.map(t => (
                <div
                  key={t.id}
                  className={cn(
                    "flex items-center gap-4 p-3.5 rounded-xl border transition-colors group",
                    t.is_active
                ...
                >
                  <div className={cn(
                    ...
                    t.is_active ? 'bg-blue-500 animate-pulse' : t.status === 'in_progress' ? 'bg-blue-400' : 'bg-slate-300'
                  )} />
